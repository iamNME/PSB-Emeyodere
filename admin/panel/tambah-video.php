            ...
            <!-- <div class="card-header pr-4 pl-4">
              ...
              <!-- form start -->
                  ...
                  if($size < 2048000 && in_array($ext, $ekstensi)){
                    ...
                    $sql = "INSERT INTO video (id, albumv_id, video_url, thumbnail) VALUES ('$id', '$namaAlbum', '$urlVideo', '$thumbnailname')";
              ...
              <form class="form-horizontal" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data">
                <div class="card-body pr-4 pl-4">
                  ...
                  <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                      <input type="text" name="url_video" class="form-control" id="url_video" placeholder="https://www.youtube.com/watch?v=GmG4X9PGOXs" required>
                    </div>
                  </div>
									<div class="form-group row">
                    <label class="col-sm-2 col-form-label">Thumbnail</label>
                    <div class="col-sm-10 custom-file">
                      <input type="file" name="thumbnail" class="custom-file-input form-label" id="thumbnail" onChange="loadfile(event)" required>
                      <label class="custom-file-label" for="thumbnail">Choose file</label>
                    </div>
                  </div>
									<div class="form-group row hidden" id="thumbnail-row">
                    <div class="col-sm-2">

                    </div>
                    <div class="col-sm-10">
                      <img width="50%" height="250" src="" id="preview" alt="Thumbnail" style="object-fit: cover;">
                    </div>
                  </div>
                  <div class="form-group row">
                        ...
                        <option value="">---Select Album Name---</option>
                          ...
                          if(mysqli_num_rows($query) > 0){
                            while($row = mysqli_fetch_assoc($query)){
												?>
												<option value="<?= $row['id_albumv'] ?>"><?= $row['namaalbum'] ?></option>
                        <?php
                            }
                          }
                        ?>
...
<script>
  ...
  let formRow = document.getElementById('thumbnail-row');
	// let url = document.getElementById('url_video');
  
	const loadfile = (event) =>{
		// let urlVid = url.value.substr(32);
		// console.log(urlVid);
		formRow.classList.remove('hidden');
		output.src = URL.createObjectURL(event.target.files[0]);
    // output.src = `https://img.youtube.com/vi/${urlVid}/maxresdefault.jpg`;
  }
